...
/*   Updated: 2026/01/06 00:50:56 by lagrondi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_duplicate_method(int argc, char **argv)
{
	if (argc > 1)
	{
		if (!strcmp(argv[1], "0") || !strcmp(argv[1], "bit") \
			|| !strcmp(argv[1], "bits"))
			mem_block_type = 0;
		else if (!strcmp(argv[1], "1") || !strcmp(argv[1], "byte") \
			|| !strcmp(argv[1], "bytes"))
			mem_block_type = 1;
		else if (!strcmp(argv[1], "2") || !strcmp(argv[1], "word") \
			|| !strcmp(argv[1], "words"))
			mem_block_type = 2;
		else if (!strcmp(argv[1], "3") || !strcmp(argv[1], "fast") \
			|| !strcmp(argv[1], "memcpy"))
			mem_block_type = 3;
		else
...
static void	print_start_infos(t_data dt, const char **str_arr, int bui_delay_ms)
{
	printf("\n======== STARTING INFOS ===========\n");
	if (mem_block_type == 0)
		printf(" - MEM_BLOCK_TYPE = bits  --> %d ms\n", bui_delay_ms);
	else if (mem_block_type == 1)
		printf(" - MEM_BLOCK_TYPE = bytes --> %d ms\n", bui_delay_ms);
	else if (mem_block_type == 2)
		printf(" - MEM_BLOCK_TYPE = words --> %d ms\n", bui_delay_ms);
	else
		printf(" - MEM_BLOCK_TYPE = ????? --> %d ms\n - Cause real memcpy()\n", bui_delay_ms);
	(void)str_arr;
	(void)dt;
	//printf(" - String array:\n");
...
static void	print_end_infos(t_data dt, t_pos start_pos,  int duration_ms, int bui_delay_ms)
{
	...
	if (mem_block_type == 0)
		printf(" - MEM_BLOCK_TYPE = bits\n   --> %d ms\n", bui_delay_ms);
	else if (mem_block_type == 1)
		printf(" - MEM_BLOCK_TYPE = bytes\n   --> %d ms\n", bui_delay_ms);
	else if (mem_block_type == 2)
		printf(" - MEM_BLOCK_TYPE = words\n   --> %d ms\n", bui_delay_ms);
	else
		printf(" - MEM_BLOCK_TYPE = ????? (but real memcpy())\n   --> %d ms\n - Cause real memcpy()\n", bui_delay_ms);
...
int	main(int argc, char **argv)
{
	...
	const char	*str_arr[] = { \
		...
		"101001111000001111000000111100000111100011110111111110001", \
		"101000000000000000000000000000000000000000000000000000001", \
		"10100000000000000000000000P000000000000000000000000000001", \
		"101000000000000000000000000000000000000000000000000000001", \
		"101001111000001111000000111100000111100011110111111110001", \
		"101001111000001111000000111100000111100011110111111110001", \
		"101001111000001111000000111100000111100011110111111110001", \
	...
	set_duplicate_method(argc, argv);
