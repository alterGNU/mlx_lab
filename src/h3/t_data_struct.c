...
/*   Updated: 2026/01/06 00:43:44 by lagrondi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_data	init_data(const char **str_arr)
{
	...
	if (win_x <= 0 || win_y <= 0)
		return (dt);
	if (mem_block_type == 1)
		dt.win_ptr = mlx_new_window(dt.mlx_ptr, win_x, win_y, WIN_TITLE_1);
	else if (mem_block_type == 2)
		dt.win_ptr = mlx_new_window(dt.mlx_ptr, win_x, win_y, WIN_TITLE_2);
	else if (mem_block_type == 3)
		dt.win_ptr = mlx_new_window(dt.mlx_ptr, win_x, win_y, WIN_TITLE_3);
	else
		dt.win_ptr = mlx_new_window(dt.mlx_ptr, win_x, win_y, WIN_TITLE_0);
	if (!dt.win_ptr)
		return (dt);
