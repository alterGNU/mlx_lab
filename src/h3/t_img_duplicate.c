...
/*   Updated: 2026/01/06 00:20:08 by lagrondi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	dup_t_img_by_bits(t_img *src, t_img *dst)
{
	...
	ft_memcpy(dst->addr, src->addr, length);
	return (1);
}
...
int	dup_t_img_by_bytes(t_img *src, t_img *dst)
{
	...
	copy_t_img_metadata(src, dst);
	length = (size_t)(src->size_line * src->height);
	ft_memcpy_by_bytes(dst->addr, src->addr, length);
	return (1);
}
...
int	dup_t_img_memcpy(t_img *src, t_img *dst)
{
	...
	copy_t_img_metadata(src, dst);
	length = (size_t)(src->size_line * src->height);
	memcpy(dst->addr, src->addr, length);
	return (1);
}
