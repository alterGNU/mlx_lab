...
/*   Updated: 2026/01/03 20:00:41 by lagrondi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

/**
 * Display a simple 2D maze with XPM images as tiles, using a buffer image.
 * 
 * - IMPLEMENTATION:
 *   - Load floor and wall images from XPM files.
 *   - Create a maze from a string array.
 *   - Create a buffer image to draw the entire maze in memory.
 *   - Draw the buffer image to the window in one go.
 *   - Clean exit on [ESC] key or window close (using mlx_loop_end()).
 *   - Display how long the program ran and how many images were drawn.
 * - OBSERVATIONS:
 *  - This approach is more efficient for larger mazes since it minimizes
 *    the number of draw calls to the window by using a buffer image.
 */
...
int	draw_buffer_image(t_data *dt)
{
	...
	while (++i < tot_len)
	{
		...
		y = i / dt->maze.width;
		if (dt->maze.mat[i] == 0)
			res = mlx_xpm_cpy_src_in_dst(&dt->img_floor, &dt->img_buffer, \
				x * TILE_X, y * TILE_Y);
		else if (dt->maze.mat[i] == 1)
			res = mlx_xpm_cpy_src_in_dst(&dt->img_wall, &dt->img_buffer, \
				x * TILE_X, y * TILE_Y);
		else
			res = 1;
		if (res)
	...
	mlx_put_image_to_window(dt->mlx_ptr, dt->win_ptr, dt->img_buffer.img_ptr, \
		0, 0);
	dt->img_drawn++;
	return (0);
}
